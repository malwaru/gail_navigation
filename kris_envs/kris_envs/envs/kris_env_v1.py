...
class KrisEnvRgb(gym.Env,Node):
    def __init__(self):
        ...
        self.observation_space = spaces.Tuple((
                                        spaces.Box(low=-100.0, high=100.0, shape=(1,7), dtype=np.float32),
                                        spaces.Box(low=-np.inf, high=np.inf, shape=(1280, 8, 10), dtype=np.float32),
                                        spaces.Box(low=-np.inf, high=np.inf, shape=(238,318), dtype=np.float32)
        ))
